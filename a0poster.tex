...
\newcommand{\READABLE}{\@setfontsize\READABLE{32pt}{37pt}} % 本文用 (読みやすく)
% キャプションのフォントサイズを本文と同じにする
\renewcommand{\READABLE}{%
    \@setfontsize\READABLE{32pt}{37pt}%
    \let\normalsize\READABLE
}
...
\begin{columns}
    ...
    \autoblock{背景}{
        \begin{itemize}
            ...
            \item DNS ベースのリクエストナビゲーションでは、一般的にクライアントの IP アドレスを手掛かりに最適なコンテンツ配信サーバを選択する
            \item 名前解決はクライアント（利用者）、フルリゾルバ（仲介）、権威 DNS サーバを用いて、ドメイン名を IP アドレスに変換する技術である。（図\the\numexpr\value{figure}+1\relax）
            ...
            \item フルリゾルバの IP アドレスを基に最適なコンテンツ配信サーバを選択している。従来は、これで適切な選択が実現できていた。
            \item これは「フルリゾルバはクライアントとネットワーク的に近い」という前提があったため実現されていた
            \item この前提が成り立っているため、クライアントの IP アドレスではなく、フルリゾルバの IP アドレスを用いることで最適なサーバ選択が実現できる
    ...
    \autoblock{ECS(EDNS Client Subnet)}{
            \begin{itemize}
                    ...
                    \item Public DNS を利用している状況でも、クライアントのサブネット情報に基づいて最適なコンテンツ配信サーバを選択できるようになる
            \end{itemize} 
    }
    ...
    \autoblock{研究目的}{
        \begin{itemize}
            ...
            \item 影響を軽減するための解決策を提案し、その効果を評価する
        \end{itemize}
    }
    ...
    \autoblock{影響の調査}{
        ...
        \subsec{環境}
        \begin{itemize}
            \item 調査のために ECS 対応フルリゾルバを構築し、クライアントはから調査用フルリゾルバに名前解決を要求する（図\the\numexpr\value{figure}+1\relax）
            \item クライアントは /32 のサブネット情報を付与して名前解決する。
            \item 調査用フルリゾルバは受け取った /32 を、フルリゾルバ別に設定したサブネット情報の長さ（/16, /22, /24）に丸めて名前解決をする
            \item 比較用として ECS 無しのフルリゾルバも用意する
            \item サブネット情報の長さによって名前解決時間・メモリ使用量に与える影響を比較する
            \item 5,000件の実在するドメイン名と8種類のサブネット情報を組み合わせて名前解決する
            \item サブネット情報は連続サブネットとランダムサブネットの2パターンで実施して違いを比較する
            \item 実験を5回繰り返し、平均値を算出する
        \end{itemize}
        \begin{center}
        ...
        \subsec{調査結果}
        \begin{itemize}
            \item 図\the\numexpr\value{figure}+1\relax および 図\the\numexpr\value{figure}+2\relax に示すとおり、丸めるサブネット情報が短い（/16）ほどメモリ使用量は小さく、長い（/24）ほど大きい傾向。
            \item 連続サブネット（図\the\numexpr\value{figure}+1\relax）はサブネット情報の長さによってメモリ使用量は異なる。対して、ランダムサブネット（図\the\numexpr\value{figure}+2\relax）は ECS無し, ECS/22 と ECS /16, /24で二極化した。
            \item 図\the\numexpr\value{figure}+3\relax の平均名前解決時間は、どちらも /22が最も長く、ECS を利用するフルリゾルバは ECS 無しと比べて40ミリ秒程度名前解決時間が増加している。
        \end{itemize}
        \begin{center}
            \begin{minipage}{0.48\linewidth}
                ...
                \refstepcounter{figure}
                {\normalsize 図\thefigure\quad 連続サブネットのメモリ遷移}
            \end{minipage}\hfill
            \begin{minipage}{0.48\linewidth}
                \centering
                \includegraphics[width=\linewidth]{img/random-subnet-graph.png}
                \refstepcounter{figure}
                {\normalsize 図\thefigure\quad ランダムサブネットのメモリ遷移}
            \end{minipage}
        \end{center}
        \begin{center}
            \begin{minipage}{0.75\linewidth}
                ...
                \refstepcounter{figure}
                {\normalsize 図\thefigure\quad 平均名前解決時間(ms)}
            \end{minipage}
        \end{center}
    ...
    \autoblock{今後の展望}{
        \begin{itemize}
            \item 他のフルリゾルバでも調査を行う
            \begin{itemize}
                \item 今回フルリゾルバのソフトウェアは Unbound を使用したので、 PowerDNS Recursor を使って同じ環境で実験する
            \end{itemize}
            \item 調査結果を踏まえ、メモリ使用量と名前解決時間を軽減するための解決策を提案し、その効果を評価する
            \begin{itemize}
                \item ECS 有効ドメインと無効ドメインを判別して複数フルリゾルバで処理する方法を今後実験する予定である
            \end{itemize}
        \end{itemize}
    }
